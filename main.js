...
importFile.addEventListener("change", async (e) => {
  ...
  reader.onload = async (ev) => {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      // Better CSV parsing that handles empty columns
      const values = [];
      let current = "";
      let inQuotes = false;
      ...
      for (let j = 0; j <= line.length; j++) {
        ...
        if (char === '"') {
          inQuotes = !inQuotes;
        } else if ((char === ',' || j === line.length) && !inQuotes) {
      ...
      if (values.length < 12) {
        // console.log(`Row ${i + 1}: SKIPPED - only ${values.length} columns (need 12). Values:`, values);
        skippedShortRow++;
        continue;
      }
      ...
      if (assets.some(a => a.tag === tag || a.serial === serial)) {
        // console.log(`Row ${i + 1}: SKIPPED - duplicate tag "${tag}" or serial "${serial}"`);
        skippedDuplicate++;
        continue;
      }
      ...
      if (success) {
        added++;
        // console.log(`Row ${i + 1}: ADDED - tag "${tag}"`);
      } else {
        failed++;
        // console.log(`Row ${i + 1}: FAILED - Supabase error for tag "${tag}"`);
      }
    ...
    // console.log("Skipped (no columns):", skippedNoCols);
    // console.log("Skipped (short row):", skippedShortRow);
    // console.log("Skipped (duplicate):", skippedDuplicate);
    // console.log("Failed (Supabase error):", failed);
